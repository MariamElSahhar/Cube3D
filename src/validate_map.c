...
/*   Updated: 2024/02/12 10:35:06 by melsahha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_map_line (char* line) {
    ...
    if ((line[i] && line[i] == '\n') || !line[i])
        return (0);
    while (line[i] && line[i] != '\n') {
        if (!(line[i] == '1' || line[i] == '0' || line[i] == 'W' || line[i] == 'E' || line[i] == 'S' || line[i] == 'N' || line[i] == ' '))
            return (0);
        i++;
    }
    return (1);
...
int read_f_c(t_map* map, char type, char *data) {
    ...
    res = 0;
    if ((type == 'F' && map->floor[0] != -1) || (type == 'C' && map->ceiling[0] != -1)) 
        return (res);
    rgb = ft_split(data, ',');
